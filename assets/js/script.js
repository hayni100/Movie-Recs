...
function getStreamsByIMDBID(IMDBID){//uses Get by IMDb ID which is an option from the MDBList API (paid for by Rhys)
	var getByIMDBidURL = 
	...
	.then(function (imdbObject) {
		...
		console.log(imdbObject+" is the IMBDObject");
			...
			//if (typeof(imdbObject.streams[0].name) === 'undefined') {//if there is no first stream source (movie isn't streaming yet)
			...
			//}
			 trailerPath = imdbObject.trailer
			document.querySelector("#vote_average2").textContent = trailerPath;
			}
